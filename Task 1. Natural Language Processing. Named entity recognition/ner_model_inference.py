...
print(f"""
Created DataFrame
Test dataset length:  {len(test_df)} instances.
""")
...
def calculate_metrics(total_correct_class_2, total_class_2, total_predicted_class_2):
    """
    Calculate precision, recall, F1-score, and accuracy metrics.

    Arguments:
    - total_correct_class_2 (int): Total number of correctly predicted class 2 tokens.
    - total_class_2 (int): Total number of actual class 2 tokens.
    - total_predicted_class_2 (int): Total number of predicted class 2 tokens.
    ...
    """
    precision = total_correct_class_2 / total_predicted_class_2 if total_predicted_class_2 > 0 else 0.0
    recall = total_correct_class_2 / total_class_2 if total_class_2 > 0 else 0.0
    f1_score = 2 * (precision * recall) / (precision + recall) if (precision + recall) > 0 else 0.0
    accuracy = total_correct_class_2 / total_class_2 if total_class_2 > 0 else 0.0
...
def eval_step(model, dataloader, device):
    ...
    with torch.inference_mode():
        for batch in dataloader:
            ...
            # Calculate accuracy for class 2 in this batch
            ...
            batch_predicted_class_2 = (predicted_labels == 2).sum().item()

            total_correct_class_2 += batch_correct_class_2
            total_class_2 += batch_class_2
            total_predicted_class_2 += batch_predicted_class_2

    precision, recall, f1_score, accuracy = calculate_metrics(total_correct_class_2, total_class_2, total_predicted_class_2)
...
print("\n")
metrics = eval_step(model=loaded_ner_model, dataloader=test_dataloader, device=device)
print("Calculated metrics:")
